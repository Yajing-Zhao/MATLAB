...
%
% j should be a column vector
% of indices of grid points, values u(x0 + j*h)
...
disp(' ')
disp('For smooth u,')
disp(sprintf('       Error = %g * h^%i*u^(%i)(x0) + %g * h^%i*u^(%i)(x0) + ...',err0,n-k,n,err1,n-k+1,n+1))

disp(' ')


% Modifications to obtain fractions for the coefficients c(i).
...
for i=1:n-1
  if j(i) < 0
    fprintf('%s * u(x0%i*h) + ',strtrim(rats(c(i))),j(i))
  elseif j(i) == 0
    fprintf('%s * u(x0) + ',strtrim(rats(c(i))))
  else
    fprintf('%s * u(x0+%i*h) + ',strtrim(rats(c(i))),j(i))
  end
end

disp(sprintf('%s * u(x0+%i*h) ]   ',strtrim(rats(c(n))),j(n)))

% Dominant terms in truncation error with a fraction as a coeffcient:

disp(' ')
disp('For smooth u,')
disp(sprintf('       Error = %s * h^%i*u^(%i)(x0) + %s * h^%i*u^(%i)(x0) + ...',strtrim(rats(err0)),n-k,n,strtrim(rats(err1)),n-k+1,n+1))
